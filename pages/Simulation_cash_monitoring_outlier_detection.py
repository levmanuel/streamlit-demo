...
with col1:
    ...
    date_delta_7 = np.where(np.abs(df["date_delta"])> 7,1,0)
    date_delta_30 = np.where(np.abs(df["date_delta"])> 30,1,0)
    fin_semaine = df["Booking Date"].apply(lambda x: (x.weekday() >= 5)).astype(int)
